...
@app.route('/')
def index(offset=0):
    num_of_items = Item.query.count()  # 发布的物品的数量
    print(num_of_items)
    pages = num_of_items / \
        page_size if num_of_items % page_size == 0 else num_of_items / page_size + 1

    showed_items = Item.query.filter(
        Item.isclosed == 0).paginate(int(offset+1), int(page_size), False)
    items = {
        'items': showed_items.items,
        'num_of_items': list(
            range(
                int(pages)))}
...
def index_(offset):
    ...
    pages = num_of_items / \
        page_size if num_of_items % page_size == 0 else num_of_items / page_size + 1
    showed_items = Item.query.filter(
        Item.isclosed == 0).paginate(page=int(int(offset) + 1), per_page=int(page_size), error_out=False)
    items = {
        'items': showed_items.items,
        'num_of_items': list(
            range(
...
def login():
    if request.method == 'GET':
        ...
        if user and user.checkPassWord(password):
            ...
            return redirect(url_for('index_', offset=0))
        else:
...
def detail(itemid):
    ...
    inter = Interest.query.filter(
        Interest.userid == session.get('user_id'),
        Interest.itemid == itemid).first()
    if inter:
        flag = True
    else:
        flag = False
...
def search():
    ...
    if q:
        items = Item.query.filter(
            Item.name.contains(q)).order_by('-create_time').all()
    else:
...
def deleteitem(itemid):
    item = Item.query.filter(Item.id == itemid).first()
    interests = Interest.query.filter(Interest.itemid == itemid).all()
    comments = item.comments
...
def deleteitem_root(itemid):
    item = Item.query.filter(Item.id == itemid).first()
    inters = Interest.query.filter(Interest.itemid == itemid).all()
    for inter in inters:
        db.session.delete(inter)
    comments = item.comments
    for comment in comments:
